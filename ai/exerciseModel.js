...
const encodedLabels = trainingData.map((data) =>
  oneHotEncode(getActivityIndex(data.preferActivity), 3)
);
...
const inputTensor = tf.tensor2d(
  trainingData.map((data) => [
    parseFloat(data.dailyCaloriesBurnGoal),
    getActivityIndex(data.preferActivity),
  ])
);
const labelTensor = tf.tensor2d(
  trainingData.map((data) => [
    Number(getActivityIndex(data.preferActivity)),
    3,
  ]),
  "float32"
);
